...
.. list-table::
    ...
    * - OIDC or OAuth2 only
      - Canonical Identity Platform (Ory stack)
    * - The highest scalability
      - Canonical Identity Platform (Ory stack)
    * - A deeply customized login experience or custom authentication flow steps
      - Canonical Identity Platform (Ory stack)
    * - An all-in-one identity provider with a built-in administrative console
      - Charmed Authentik
    * - LDAP, or LDAP and OIDC from one control plane
      - Charmed Authentik
...
:doc:`GLAuth </reference/charms/glauth>` is a lightweight, read-only LDAP server interface. It provides a simple LDAP endpoint suitable for local development, integration testing, and lightweight staging environments.

...
.. code-block:: bash
    ...
    juju integrate sssd:ldap authentik-ldap-outpost:ldap

.. note::
    ``sssd`` is a subordinate machine charm: it runs on machines alongside a principal application, not on Kubernetes. From a machine model, consume the ``ldap`` offer published by the Authentik solution and integrate against it instead:

    .. code-block:: bash

        juju integrate sssd:ldap admin/authentik.ldap

    Kubernetes-native LDAP clients can integrate with the outpost's ``ldap`` endpoint directly in the same model.
...
1. **Retrieve the Dynamic Credentials**:
   ...
   .. code-block:: bash
       ...
       juju show-unit sssd/0 --endpoint ldap

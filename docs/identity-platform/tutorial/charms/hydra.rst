...
Deploy, configure, integrate, and manage Charmed Hydra on any Kubernetes cloud
------------------------------------------------------------------------------

Hydra requires a way to persist data, in the case of our charm we enforce the usage of a ``postgreSQL`` database

As mentioned, we need a persistent way to store ``Hydra`` data, we are going to be using the `postgresql-k8s charm <https://charmhub.io/postgresql-k8s>`_ 
...
.. code-block:: shell
    ...
    hydra  microk8s-localhost  microk8s/localhost  3.6.3    unsupported  15:57:02+01:00
...
.. code-block:: shell
    ...
    Model  Controller          Cloud/Region        Version  SLA          Timestamp
    hydra  microk8s-localhost  microk8s/localhost  3.6.3    unsupported  16:02:36+01:00
